...
Cont4Wheels::Cont4Wheels(const std::string& name, const lpzrobots::OdeConfig& odeconfig)
  : AbstractController(name, "1.0"), odeconfig(odeconfig)  { }


double Cont4Wheels::couplingRod(double x_tar, double phi) 
{ // defines transmission of power to the wheels
  return k * sin(phi) * ( x_tar - cos(phi) );
}

double Cont4Wheels::y(double phi, double deltaPhi)
{ // sigmoidal funcion,  where x=cos(phi-dphi)
  double x = cos(phi-deltaPhi);
  return 1./( 1.+exp( a*(b-x) ) );
}

double Cont4Wheels::y(double x)
{ // sigmoidal funcion  
  return 1./( 1.+exp( a*(b-x) ) );
}
...
void Cont4Wheels::stepNoLearning(const sensor* sensors, int number_sensors,
...
void Cont4Wheels::step(const sensor* sensors, int sensornumber,
...
void Cont4Wheels::init(int sensornumber, int motornumber, RandGen* randGen) {
...
int Cont4Wheels::getSensorNumber() const {
  return nSensors;
}


int Cont4Wheels::getMotorNumber() const {
  return nMotors;
}


bool Cont4Wheels::store(FILE* f) const {
...
bool Cont4Wheels::restore(FILE* f) {
